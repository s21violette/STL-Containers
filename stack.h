...
namespace s21 {
template <class T>
class stack : public AbstractList<T>::AbstractList {
    using value_type = T;
 ...
 public:
    stack() : AbstractList<T>::AbstractList() {}
    explicit stack(size_type n) : AbstractList<T>::AbstractList(n) {}
    stack(std::initializer_list<value_type> const& items)
        : AbstractList<T>::AbstractList(items) {}
    explicit stack(const AbstractList<value_type>& l)
        : AbstractList<T>::AbstractList(l) {}
    explicit stack(AbstractList<value_type>&& l) : AbstractList<T>::AbstractList(l) {}
